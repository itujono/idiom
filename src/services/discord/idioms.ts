...
export class IdiomsDiscordService extends BaseDiscordService {
  ...
  async sendIdioms(idioms: Idiom[]): Promise<void> {
    ...
    for (const [index, message] of messages.entries()) {
      const prefix = messages.length > 1 ? `Part ${index + 1}/${messages.length}\n\n` : '';
  ...
  private formatIdiomsMessages(idioms: Idiom[]): string[] {
    ...
    while (currentIdiomIndex < idioms.length) {
      ...
      const isExceedingLimit = currentMessage.length + idiomContent.length > this.MAX_MESSAGE_LENGTH;
  ...
  private formatSingleIdiom(idiom: Idiom, index: number): string {
    ...
    if (idiom.examples) content += `📝 Example:\n${idiom.examples}\n\n`;

    return content;
  }

  async testWebhook(): Promise<void> {
    await this.sendMessage('Test message from Daily Idioms', 'Daily Idioms Test');
  }
}
