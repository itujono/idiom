...
export abstract class BaseDiscordService {
  ...
  protected async sendMessage(content: string, username: string): Promise<void> {
    const startTime = Date.now();

    try {
      logger.info({ contentLength: content.length }, `Sending ${this.serviceName.toLowerCase()} message`);
      ...
      logger.info({ duration: `${duration}ms` }, `Successfully sent ${this.serviceName.toLowerCase()} message`);
