import type { Idiom } from "../types";

export const idioms: Idiom[] = [
  {
    phrase: "Bite off more than you can chew",
    meaning: "To take on more responsibility than you can manage",
    examples: [
      {
        english:
          "I think Jenny bit off more than she could chew when she volunteered to organize the entire event by herself.",
        indonesian:
          "Saya rasa Jenny terlalu memaksakan diri ketika dia menawarkan diri untuk mengorganisir seluruh acara sendirian.",
      },
      {
        english:
          "Don't bite off more than you can chew by taking on three projects at once.",
        indonesian:
          "Jangan terlalu memaksakan diri dengan mengambil tiga proyek sekaligus.",
      },
    ],
  },
  {
    phrase: "Hit the nail on the head",
    meaning: "To describe exactly what is causing a situation or problem",
    examples: [
      {
        english:
          "You hit the nail on the head when you said the project failed due to poor communication.",
        indonesian:
          "Kamu tepat sekali ketika mengatakan proyek itu gagal karena komunikasi yang buruk.",
      },
      {
        english:
          "The analyst hit the nail on the head with his market predictions.",
        indonesian: "Analis itu sangat tepat dengan prediksi pasarnya.",
      },
    ],
  },
  {
    phrase: "Hit the ground running",
    meaning:
      "To start something and proceed at a fast pace with great enthusiasm",
    examples: [
      {
        english:
          "She hit the ground running in her new job and impressed everyone with her quick progress.",
        indonesian:
          "Dia langsung bekerja dengan sangat baik di pekerjaan barunya dan membuat semua orang terkesan dengan kemajuan cepatnya.",
      },
      {
        english:
          "We need to hit the ground running with this project as the deadline is tight.",
        indonesian:
          "Kita perlu segera memulai proyek ini dengan cepat karena tenggat waktunya sangat ketat.",
      },
    ],
  },
  {
    phrase: "Break the ice",
    meaning:
      "To do or say something to reduce tension or make people feel more comfortable",
    examples: [
      {
        english:
          "He told a joke to break the ice at the beginning of the meeting.",
        indonesian:
          "Dia menceritakan lelucon untuk mencairkan suasana di awal pertemuan.",
      },
      {
        english:
          "The team-building activities helped break the ice between the new employees.",
        indonesian:
          "Kegiatan team building membantu mencairkan suasana di antara karyawan baru.",
      },
    ],
  },
  {
    phrase: "Cut corners",
    meaning:
      "To do something in the easiest or cheapest way, often sacrificing quality",
    examples: [
      {
        english:
          "The builder cut corners when renovating the house, and now we're having problems.",
        indonesian:
          "Pembangun mengambil jalan pintas saat merenovasi rumah, dan sekarang kita mengalami masalah.",
      },
      {
        english:
          "If you cut corners on quality control, it will cost more in the long run.",
        indonesian:
          "Jika kamu mengambil jalan pintas dalam kontrol kualitas, akan lebih mahal biayanya di kemudian hari.",
      },
    ],
  },
  {
    phrase: "Pull someone's leg",
    meaning: "To joke with someone by telling them something untrue",
    examples: [
      {
        english:
          "Don't worry, I'm just pulling your leg! I didn't really lose your phone.",
        indonesian:
          "Jangan khawatir, aku hanya bercanda! Aku tidak benar-benar menghilangkan ponselmu.",
      },
      {
        english: "He's always pulling my leg with his crazy stories.",
        indonesian: "Dia selalu menggodaku dengan cerita-cerita gilanya.",
      },
    ],
  },
  {
    phrase: "Under the weather",
    meaning: "Feeling ill or unwell",
    examples: [
      {
        english:
          "I'm feeling a bit under the weather today, so I might stay home.",
        indonesian:
          "Aku merasa sedikit tidak enak badan hari ini, jadi mungkin aku akan tinggal di rumah.",
      },
      {
        english: "She's been under the weather since last week.",
        indonesian: "Dia sedang tidak enak badan sejak minggu lalu.",
      },
    ],
  },
  {
    phrase: "Cost an arm and a leg",
    meaning: "To be extremely expensive",
    examples: [
      {
        english: "That new car must have cost an arm and a leg!",
        indonesian: "Mobil baru itu pasti harganya sangat mahal!",
      },
      {
        english: "Living in the city center costs an arm and a leg these days.",
        indonesian: "Tinggal di pusat kota sangat mahal akhir-akhir ini.",
      },
    ],
  },
  {
    phrase: "Beat around the bush",
    meaning: "To avoid talking about something directly",
    examples: [
      {
        english: "Stop beating around the bush and tell me what happened!",
        indonesian:
          "Berhenti berbelit-belit dan katakan padaku apa yang terjadi!",
      },
      {
        english:
          "He spent ten minutes beating around the bush before asking for a raise.",
        indonesian:
          "Dia menghabiskan sepuluh menit berbelit-belit sebelum meminta kenaikan gaji.",
      },
    ],
  },
  {
    phrase: "Once in a blue moon",
    meaning: "Very rarely; almost never",
    examples: [
      {
        english: "I only eat fast food once in a blue moon.",
        indonesian: "Saya hanya makan makanan cepat saji sangat jarang.",
      },
      {
        english:
          "We see each other once in a blue moon since she moved abroad.",
        indonesian:
          "Kami sangat jarang bertemu sejak dia pindah ke luar negeri.",
      },
    ],
  },
];
